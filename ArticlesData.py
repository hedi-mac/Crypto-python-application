...
class ArticlesData : 
    ...
    def __init__(self, monitored_tickers):
        ...
        self.SEARCH_ENGINE_ID = self.config['google']['search_engine_id']
    ...
    def get_urls(self, ticker, source, pages):
        hrefs = []
        start = (pages - 1) * 10 + 1
        url = f"https://www.googleapis.com/customsearch/v1?key={self.API_KEY}&cx={self.SEARCH_ENGINE_ID}&q={source}+{ticker}&tbm=nws&lr=lang_en&start={start}&dateRestrict=d1"
    ...
    def search_for_stock_new_urls(self, ticker):
        hrefs = self.get_urls(ticker, "yahoo", 10)
        hrefs.extend(self.get_urls(ticker, "bloomberg", 2))
        print(hrefs)
        return hrefs
    """

    def search_for_stock_new_urls(self, tickers):
        search_url = "https://www.google.com/search?q=yahoo+finance+{}&tbm=nws&tbs=qdr:d7&lr=lang_en".format(tickers)
    ...
    def getData(self):
        raw_urls = {ticker:self.search_for_stock_new_urls(ticker) for ticker in self.monitored_tickers}
